...
void sobelEdgeDetection(unsigned char* input, unsigned char* output, int width, int height, int channels) {
    ...
    // Loop through each pixel (except the border pixels)
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            ...
            for (int i = -1; i <= 1; i++) {
                for (int j = -1; j <= 1; j++) {
                    int pixel = input[((y + i) * width + (x + j)) * channels];
            ...
            int threshold = 128; // Threshold value for edge detection
...
void write_jpeg_file(const char* filename, unsigned char* image_buffer, int width, int height, int channels, int quality) {
    ...
    cinfo.cinfo.input_components = 1; // Grayscale
...
int main() {
    ...
    write_jpeg_file("/home/hpcap/Desktop/image_process/Image-Processing/data/output/edge_detectionc_edge_detection_output.jpg", edge_image, width, height, 1, 100);
    free(edge_image);
